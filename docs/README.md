...
- **Cursor tracking**: Automatically types at the position where you started recording
- **Visual feedback**: Cursor highlight when recording starts
- **Audio feedback**: Soft beep when recording begins
- **Background mode**: Run silently with system tray icon
...
Run the assistant in the background with a system tray icon:

```bash
# Install new dependencies first
pip install pywin32 pystray pillow

# Then run in background mode
pythonw start_background.pyw
```

Or simply **double-click** `start_background.pyw` in File Explorer.

**Background Mode Features:**
- ✅ Runs without console window
- ✅ System tray icon for easy access
- ✅ Right-click icon for status and exit
- ✅ All logging saved to `vokey_YYYYMMDD.log`

**System Tray Menu:**
- `🎙️ Voice Assistant - Running` - Status indicator
- `Show Status` - View current state and hotkey
- `Exit` - Stop the assistant

### Option 2: Console Mode

For debugging or testing, run in console mode:
...
1. Start the assistant (background or console mode)
2. Open any text editor (Notepad, Word, VS Code, browser, etc.)
3. Place your cursor where you want text to appear
4. **Press** `Ctrl + M` once to start recording
   - You'll see a blue highlight around your cursor
   - You'll hear a soft beep
   - Your cursor position is saved
5. Speak your message (you can move your mouse while speaking)
6. **Press** `Ctrl + M` again to stop
7. Wait 1-3 seconds for transcription
8. Text will be typed at your original cursor position

**✨ New: Position Tracking**
The app stores your cursor position when you START recording, not when you stop. This means you can move your mouse during recording, and the text will still be typed where you initially placed the cursor!

### Example Workflow

```
1. Open Notepad
2. Click in the text area (cursor is here)
3. Press Ctrl+M → See blue highlight + hear beep
4. Move mouse somewhere else (doesn't matter!)
5. Say: "Hello, this is a test"
6. Press Ctrl+M again
7. Text appears at your ORIGINAL cursor position: Hello, this is a test
```
...
### Cursor highlight not appearing
- Ensure you have tkinter installed (comes with Python by default)
- Check if another application is blocking overlays
- The highlight lasts 500ms - it might disappear quickly

### No beep sound
- Check system volume
- Verify sound is not muted
- Some systems may have beep sounds disabled in BIOS

### Text types at wrong location
- Cursor position is captured when you PRESS Ctrl+M (start recording)
- Ensure the cursor is in the correct location before starting
- The app will click at the stored position before typing
